...
Ask if they received the original request and if they can provide a quote.`;
  ...
  // ============================================
  // MARKETPLACE LISTINGS (OfferUp-style upload)
  // ============================================
  listings: router({
    ...
    mine: protectedProcedure.query(({ ctx }) =>
      db.getListings({ sellerId: ctx.user.id, status: undefined as any })
    ),
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        isFirmOnPrice: z.boolean().default(false),
        ...
        photos: z.array(z.object({
          fileData: z.string(),   // base64 (no data URL prefix)
          mimeType: z.string(),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const { id: listingId } = await db.createListing({
          sellerId: ctx.user.id,
          title: input.title,
          price: input.price.toFixed(2),
          isFirmOnPrice: input.isFirmOnPrice,
          category: input.category,
        ...
        for (let i = 0; i < input.photos.length; i++) {
          ...
          const key = `listings/${ctx.user.id}/${listingId}/${nanoid()}.${ext}`;
        ...
        await createAuditLog(ctx.user.id, 'create', 'listing', listingId, input.title);
        return { id: listingId };
      }),
    ...
    updateStatus: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.updateListingStatus(input.id, input.status);
        await createAuditLog(ctx.user.id, 'update', 'listing', input.id, input.status);
        return { success: true };
      }),
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await createAuditLog(ctx.user.id, 'delete', 'listing', input.id);
        return { success: true };
      }),
  }),
});
