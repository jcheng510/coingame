...
export async function addListingPhoto(data: InsertListingPhoto) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(listingPhotos).values(data);
  return { id: result[0].insertId };
}
...
export async function getListings(filters?: {
  ...
  const rows = conditions.length > 0
    ? await db.select().from(listings).where(and(...conditions)).orderBy(desc(listings.createdAt))
  ...
  const byListing = new Map<number, typeof photos>();
  ...
  return rows.map(r => ({
    ...r,
    photos: (byListing.get(r.id) ?? []).sort((a, b) => a.position - b.position),
  }));
}
...
export async function getListingById(id: number) {
  ...
  return { ...row, photos: photos.sort((a, b) => a.position - b.position) };
}
